        ...
        @if ($pedido->estado_pago !== 'pagado' && $pedido->estado_pedido !== 'cancelado')
            <div class="mb-8 bg-[#FFF9F0] border border-[#B8500C]/20 rounded-2xl p-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 shadow-sm">
                <div class="space-y-1">
                    ...
                    <p class="text-xs sm:text-sm text-[#8A7A71]">Usa la pasarela simulada para continuar con el despacho de tus libros.</p>
                </div>
                <a href="{{ route('pagos.create', $pedido) }}" class="w-full sm:w-auto text-center bg-[#B8500C] hover:bg-[#963F07] text-white px-6 py-3 rounded-full text-sm font-semibold transition-colors shadow-sm whitespace-nowrap">
                    Pagar pedido
                </a>
            </div>
        @endif
