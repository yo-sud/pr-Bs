                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-5 py-4 whitespace-nowrap text-center text-sm font-medium">
                            <div class="flex items-center justify-center space-x-3">
                                ...
                                {{-- Formulario de desactivación: Cambia el estado a inactivo mediante DELETE simulado --}}
                                <form action="{{ route('admin.usuarios.destroy', $usuario->id) }}" method="POST" onsubmit="return confirm('¿Estás seguro de que deseas desactivar este usuario?');" class="inline-block">
                                    @csrf
                                    @method('DELETE')
                                    
                                    <button type="submit" class="text-orange-500 hover:text-orange-700 transition-colors" title="Desactivar usuario">
                                        {{-- NUEVO ÍCONO: Un candado que representa la desactivación de la cuenta --}}
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
